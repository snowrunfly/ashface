...
class base {
	...
	function base() {
		$this->init_var();
		echo 'init vars success <br />';
		$this->init_db();
		echo 'init db success <br />';
		//$this->init_cache();
		//$this->init_app();
		//$this->init_user();
		//$this->init_template();
	...
	function init_var() {
		...
		include_once ASH_ROOT.'./view/main.lang.php';
		$this->lang = &$lang;
	}
	...
	function init_db() {
		require_once ASH_ROOT.'lib/db.class.php';
	...
	function init_user() {
		if(isset($_COOKIE['ash_auth'])) {
			@list($uid, $username, $agent) = explode('|', $this->authcode($_COOKIE['ash_auth'], 'DECODE', ($this->input ? $this->app['appauthkey'] : ASH_KEY)));
			if($agent != md5($_SERVER['HTTP_USER_AGENT'])) {
				$this->setcookie('ash_auth', '');
			} else {
				@$this->user['uid'] = $uid;
	...
	function init_template() {
		$charset = ASH_CHARSET;
		require_once ASH_ROOT.'lib/template.class.php';
	...
	function load($model, $base = NULL, $release = '') {
		$base = $base ? $base : $this;
		echo " model should be user:".$model;
		
		if(empty($_ENV[$model])) {
			$release = !$release ? RELEASE_ROOT : $release;
			if(file_exists(ASH_ROOT.$release."model/$model.php")) {
				require_once ASH_ROOT.$release."model/$model.php";
			} else {
				require_once ASH_ROOT."model/$model.php";
			}
	...
	function get_setting($k = array(), $decode = FALSE) {
		...
		$settings = $this->db->fetch_all("SELECT * FROM ".ASH_DBTABLEPRE."settings $sqladd");
	...
	function set_setting($k, $v, $encode = FALSE) {
		...
		$this->db->query("REPLACE INTO ".ASH_DBTABLEPRE."settings SET k='$k', v='$v'");
	}

	function message($message, $redirect = '', $type = 0, $vars = array()) {
		include_once ASH_ROOT.'view/default/messages.lang.php';
	...
	function formhash() {
		return substr(md5(substr($this->time, 0, -4).ASH_KEY), 16);
	}
	...
	function &cache($cachefile) {
		...
		if(!isset($_CACHE[$cachefile])) {
			$cachepath = ASH_DATADIR.'./cache/'.$cachefile.'.php';
	...
	function serialize($s, $htmlon = 0) {
		if(file_exists(ASH_ROOT.RELEASE_ROOT.'./lib/xml.class.php')) {
			include_once ASH_ROOT.RELEASE_ROOT.'./lib/xml.class.php';
		} else {
			include_once ASH_ROOT.'./lib/xml.class.php';
		}
	...
	function setcookie($key, $value, $life = 0, $httponly = false) {
		(!defined('ASH_COOKIEPATH')) && define('ASH_COOKIEPATH', '/');
		(!defined('ASH_COOKIEDOMAIN')) && define('ASH_COOKIEDOMAIN', '');
		...
		$path = $httponly && PHP_VERSION < '5.2.0' ? ASH_COOKIEPATH."; HttpOnly" : ASH_COOKIEPATH;
		$secure = $_SERVER['SERVER_PORT'] == 443 ? 1 : 0;
		if(PHP_VERSION < '5.2.0') {
			setcookie($key, $value, $life, $path, ASH_COOKIEDOMAIN, $secure);
		} else {
			setcookie($key, $value, $life, $path, ASH_COOKIEDOMAIN, $secure, $httponly);
		}
	}

	function note_exists() {
		$noteexists = $this->db->fetch_first("SELECT value FROM ".ASH_DBTABLEPRE."vars WHERE name='noteexists'");
		if(empty($noteexists)) {
			return FALSE;
		} else {
			return TRUE;
	...
	function mail_exists() {
		$mailexists = $this->db->fetch_first("SELECT value FROM ".ASH_DBTABLEPRE."vars WHERE name='mailexists'");
		if(empty($mailexists)) {
			return FALSE;
		} else {
			return TRUE;
